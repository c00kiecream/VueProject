			...
			<div class="container">
			  <div class="col-12">	
				...
				<div class="mb-3 row">
					...
					<div class="col-md-9">
						<div class="row">
							<div class="col-lg-3 col-md-4 col-cm-2" :key="i" v-for="(m,i) in productImage.filter(c=>c.type==7)">
						...
						<input  type="file" class="form-control" accept="image/png,image/jpeg" @change="uploadFile($event.target.files, 7)">
				...
				<div class="mb-3 row">
					...
					<div class="col-md-9">
						<div class="row">
							<div class="col-lg-3 col-md-4 col-cm-2" :key="i" v-for="(m,i) in productImage.filter(c=>c.type==8)">
								<div class="position-relative">
									<img :src="`/download/${productId}/${m.path}`" class="img-fluid" />
									<div class="position-absolute top-0 end-0" style="cursor:pointer;" @click="deleteImage(m.id)">X</div>	
								</div>	
						...
						<input type="file" class="form-control" accept="image/png,image/jpeg" @change="uploadFile($event.target.files, 8)">
						<div class="alert alert-secondary" role="alert">
							<ul>
								<li>최대 5개 가능</li>
				...
				<div class="mb-3 row">
					...
					<div class="col-md-9">
						<div class="row">
							<div class="col-lg-3 col-md-4 col-cm-2" :key="i" v-for="(m,i) in productImage.filter(c=>c.type==9)">
								<div class="position-relative">
						...
						<input  class="form-control" type="file" accept="image/png,image/jpeg" @change="uploadFile($event.target.files, 9)">
						<div class="alert alert-secondary" role="alert">
							<ul>
								<li>파일 사이즈 : 5M 이하</li>
								<li>파일 확장자 : png, jpg만 가능</li>
...
export default {
	...
	let productDetail2 = await this.$api('/api/e_productDetail2', { param: [this.productId] })
	console.log('productDetail2 확인', productDetail2);
    ...
    async getProductImage () {
      this.productImage = await this.$api('/api/e_imageList', { param: [this.productId] })
	  console.log('this.productImage 확인', this.productImage);
    ...
    deleteImage(id) {
        ...
        if (result.isConfirmed) {    
          await this.$api("/api/e_imageDelete",{param:[id]});
		  this.getProductImage();
